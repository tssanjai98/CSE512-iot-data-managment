...
def insert_realtime_metrics(session, data):
    ...
    try:
        ...
        # Prepare the parameters, including a list of frozen maps for the metrics
        metrics = [
            {
                "sensor_id": metric["sensor_id"],
                "sensor_name": metric["sensor_name"],
                "value": str(metric["value"]),  # Store value as a string (TEXT)
                "units": metric["units"],
                "alert": str(metric["alert"]),  # Ensure alert is boolean
        ...
        # Use a Map for the location field (latitude and longitude)
        location = {
